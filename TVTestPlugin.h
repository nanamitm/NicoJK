	...
	*/
	...
	protected:
		...
		virtual bool OnProgramGuideCommand(
			UINT Command, const ProgramGuideCommandParam* pParam) {
			return false;
		}
		// 番組表のメニューの初期化
		virtual int OnProgramGuideInitializeMenu(
			const ProgramGuideInitializeMenuInfo* pInfo) {
			return 0;
		}
		// 番組表のメニューが選択された
		...
		virtual bool OnProgramGuideProgramDrawBackground(
			...
			const ProgramGuideProgramDrawBackgroundInfo* pInfo) {
			return false;
		}
		// 番組表の番組のメニュー初期化
		virtual int OnProgramGuideProgramInitializeMenu(
			const ProgramGuideProgramInfo* pProgramInfo,
			const ProgramGuideProgramInitializeMenuInfo* pInfo) {
			return 0;
		}
		// 番組表の番組のメニューが選択された
		virtual bool OnProgramGuideProgramMenuSelected(
			const ProgramGuideProgramInfo* pProgramInfo, UINT Command) {
			return false;
		}
#endif
#if TVTEST_PLUGIN_VERSION >= TVTEST_PLUGIN_VERSION_(0, 0, 14)
